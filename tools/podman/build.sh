#!/usr/bin/env bash
# Containerised Quartus build for the Pocket Game Gear core. Runs on the HOST
...
#
# Everything lands in build/gg/. The checked-in tree is never written to:
# Quartus runs against a copy under build/gg/work, which is also what lets us
# change fitter settings without touching files shared with upstream.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
BDIR="$REPO/build/${BUILD_NAME:-gg}"
WORK="$BDIR/work"
